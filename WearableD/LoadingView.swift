...
class LoadingView : UIView {
    ...
    //make this a class func to param these if can't use init
    func setupView () -> Void {
    ...
    func resetLineState () {
        for index in 0...4 {
            ...
            lineView.bounds.size.width = CGFloat(10);
        }
    }
    ...
    func createLoadingLines ( lineCount : Int, lineSpacing : Int)  -> Void {
        for index in 1...lineCount {
            var lineView = UIView()
            let height = 10;
            var space = (index * lineSpacing) + 10;
            //todo make width relative to passed in frame
            lineView.frame = CGRect(x: 15, y: space, width: 50, height: height);
    ...
    func startAnimation (closure: () -> ()) {
        self.animateLines(0, closure)
    }
    func animateLines (index: Int, closure: () -> ()) {
        let duration = 0.5
        ...
        UIView.animateWithDuration(duration, delay: delay, options: options, animations: {
            //lineView.bounds.size.width = CGFloat(90);
